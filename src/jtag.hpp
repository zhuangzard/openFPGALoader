...
public:
    ...
    int direct_read_write(unsigned char *tms, unsigned char *tdi, unsigned char *tdo, int len);
    ...
    int setTMS(unsigned char tms);
    int calculateTMS(unsigned char * tms_vector, int len);
    void cleanTMS();
    enum tapState_t {
        TEST_LOGIC_RESET = 0,
        RUN_TEST_IDLE = 1,
